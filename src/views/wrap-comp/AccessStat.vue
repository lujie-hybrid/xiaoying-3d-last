  ...
  >
    <div class="ast">
      ...
      <div class="ast-title">
        ...
        <p class="ast-title-text">月度入营人员类型圆环统计图详图</p>
        <v-select
          @change="getListData"
          class="ast-select"
          :select="year"
      ...
      <v-scroll class="chart-wrap">
        <ul class="ast-list">
          <li :class="{ bottom: i > 7 }" v-for="(item, i) in data" :key="i">
            ...
            ></my-pie>
            <p class="ast-tip">- {{ item.month }}月入营人员类型统计表 -</p>
            <p class="ast-month">{{ item.month }}月</p>
          </li>
        </ul>
      </v-scroll>
    </div>
...
export default {
  ...
  methods: {
    ...
    // 获取年度数据
    getListData() {
      this.$api.access.getYearMsg({ year: this.year.value }).then((res) => {
        const list = res.map((r) => {
          r.pieOption = JSON.parse(JSON.stringify(this.pieOption));
          r.pieOption.title.text = r.count;
          r.pieOption.series[0].data = r.accessRecordDtos.map((item) => ({
    ...
    // 返回
    goBack() {
      this.set_value({ key: "yearShow", val: false });
    },
  },
};
</script>
